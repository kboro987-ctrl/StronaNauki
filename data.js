...
const QUIZZES = {
  ...
  /* ===== TESTY Z KSIĄŻKI GPW (oryginalne pytania) ===== */
  ksiazka_kontrakty: {
    title: "📕 Test z książki — kontrakty terminowe",
    desc: "Oryginalny „Test sprawdzający” z książki GPW (K. Mejszutowicz).",
    icon: "📕", book: true,
    questions: [
      { q: "Kontrakt terminowy to:", o:["instrument finansowy posiadający swojego emitenta","instrument finansowy notowany na GPW należący do grupy instrumentów pochodnych","instrument, z którym związane są takie same prawa jak wynikające z akcji"], a:1, e:"Kontrakt terminowy to instrument pochodny notowany na GPW — nie ma emitenta i nie daje praw akcjonariusza." },
      { q: "W kontrakcie terminowym strony transakcji:", o:["za dodatkową opłatą mogą wycofać się z kontraktu","przyjmują na siebie zobowiązanie do rozliczenia kontraktu na warunkach z góry określonych","w terminie wygaśnięcia ustalają kwotę, wg której ostatecznie rozliczy się instrument"], a:1, e:"Obie strony są zobowiązane do rozliczenia na warunkach ustalonych z góry (ryzyko symetryczne)." },
      { q: "Które z poniższych kontraktów terminowych NIE są notowane na GPW?", o:["kontrakty terminowe na akcje","kontrakty terminowe na indeks WIG-budownictwo","kontrakty terminowe na kurs EUR/PLN"], a:1, e:"Na GPW notowane są m.in. kontrakty na akcje, indeks WIG20 i kursy walut; na indeks WIG-budownictwo — nie." },
      { q: "Nabywca kontraktu terminowego zarabia, jeżeli wartość instrumentu bazowego (z wyjątkiem kontraktów na WIBOR):", o:["rośnie","spada","nie zmienia się"], a:0, e:"Pozycja długa zyskuje przy wzroście ceny instrumentu bazowego." },
      { q: "Wystawcę kontraktu terminowego nazywamy:", o:["zajmującym pozycję długą","kupującym kontrakt","zajmującym pozycję krótką"], a:2, e:"Wystawca = sprzedający = pozycja krótka." },
      { q: "Co to jest dźwignia finansowa na kontraktach terminowych?", o:["istotna cecha kontraktów, dzięki której realizujemy wyższe stopy zwrotu niż na rynku akcji","usługa biura maklerskiego pożyczającego pieniądze na inwestycje w kontrakty","mechanizm codziennego rozliczania zysków i strat po sesji"], a:0, e:"Dźwignia wynika z wnoszenia tylko depozytu (ułamka wartości), co zwielokrotnia stopę zwrotu." },
      { q: "Ryzyko inwestowania w kontrakty terminowe:", o:["jest wyższe niż w przypadku inwestycji w akcje","zależy od tego, czy kontrakty kupujemy, czy sprzedajemy","jest identyczne jak w przypadku inwestycji w akcje"], a:0, e:"Z powodu dźwigni ryzyko kontraktów jest wyższe niż przy bezpośredniej inwestycji w akcje." },
      { q: "Co to jest depozyt zabezpieczający?", o:["środki pieniężne (lub papiery wartościowe) blokowane na rachunku inwestora, stanowiące gwarancję jego wypłacalności z tytułu zobowiązań z kontraktów","bezpieczna inwestycja polegająca na złożeniu środków na oprocentowany depozyt","środki lub papiery wpłacane na rachunek dopiero po zawarciu transakcji"], a:0, e:"Depozyt to zablokowane środki/papiery gwarantujące wypłacalność z tytułu zobowiązań kontraktowych." },
      { q: "W jaki sposób można zamknąć długą pozycję w kontrakcie terminowym?", o:["jedynym sposobem jest rozliczenie kontraktu w terminie jego wygaśnięcia","jednym ze sposobów jest sprzedaż kontraktu, czyli zawarcie transakcji odwrotnej","dopuszczalne jest, za zgodą drugiej strony, przedterminowe rozwiązanie umowy"], a:1, e:"Pozycję zamyka się transakcją odwrotną — sprzedażą takiego samego kontraktu." },
      { q: "Co to jest równanie do rynku (marking to market)?", o:["strategia otwierania pozycji zgodnych z bieżącym trendem rynkowym","zamykanie pozycji przed zakończeniem sesji giełdowej","mechanizm, zgodnie z którym codziennie po zakończeniu sesji następuje rozliczanie dziennych zysków i strat stron transakcji"], a:2, e:"Równanie do rynku = codzienne rozliczanie zysków/strat po każdej sesji." },
      { q: "Jaki jest algorytm wyznaczania dziennego kursu rozliczeniowego w kontraktach (z wyłączeniem kontraktów na WIBOR i obligacje)?", o:["kurs zamknięcia danej serii kontraktów z uwzględnieniem tzw. zasady lepszych zleceń","średnia z wartości instrumentu bazowego z ostatniej godziny notowań ciągłych oraz wartości na zamknięciu","kurs zamknięcia instrumentu bazowego z uwzględnieniem zasady lepszych zleceń"], a:0, e:"Dzienny kurs rozliczeniowy to kurs zamknięcia danej serii kontraktów (z zasadą lepszych zleceń)." },
      { q: "Jaki jest algorytm wyznaczania ostatecznego kursu rozliczeniowego w kontraktach?", o:["nie ma jednej uniwersalnej formuły — algorytm różni się w zależności od rodzaju kontraktów","zawsze jest to kurs zamknięcia serii kontraktów wygasających danego dnia","zawsze jest to kurs zamknięcia instrumentu bazowego"], a:0, e:"Ostateczny kurs rozliczeniowy zależy od rodzaju kontraktu — brak jednej uniwersalnej formuły." },
      { q: "Ile wynosi teoretyczna wartość kontraktów na akcje spółki XYZ: kurs instrumentu bazowego 22,90 zł, 67 dni do wygaśnięcia, stopa wolna od ryzyka 5%, brak dywidendy?", o:["23,11 zł","23,01 zł","23,20 zł"], a:0, e:"F = 22,90·(1 + 0,05·67/365) = 22,90·1,00918 ≈ 23,11 zł." },
      { q: "Kupiłeś kontrakt na WIG20 po 3.663 pkt i sprzedałeś po 3.700 pkt. Depozyt wstępny wynosi 10%. Zrealizowana stopa zwrotu (zaokrąglij do 0,01%)?", o:["1,01%","10,10%","101,00%"], a:1, e:"Zysk 37 pkt; depozyt = 10%·3663 = 366,3. Stopa = 37/366,3 ≈ 10,10% (efekt dźwigni)." },
      { q: "Ile można zarobić na arbitrażu z kontraktem na akcje PKN Orlen (1 kontrakt): kurs kontraktu 60,90 zł, kurs bazowego 60,00 zł, 44 dni do wygaśnięcia, stopa wolna od ryzyka 5%?", o:["53,00 zł","0,54 zł","54,00 zł"], a:2, e:"Wartość teoretyczna F=60·(1+0,05·44/365)≈60,36 zł; kontrakt (60,90) jest przewartościowany. Zysk na 100 akcji ≈ 54,00 zł." }
    ]
  },
  ksiazka_opcje: {
    title: "📕 Test z książki — opcje",
    desc: "Oryginalny „Test sprawdzający” z książki GPW (K. Mejszutowicz).",
    icon: "📕", book: true,
    questions: [
      { q: "Opcja kupna to:", o:["prawo pierwszeństwa do kupna akcji danej spółki w przypadku emisji nowych akcji","prawo do kupna od wystawcy opcji instrumentu bazowego","instrument, w którym w terminie wygaśnięcia musimy kupić od wystawcy opcji instrument bazowy"], a:1, e:"Opcja kupna (call) daje posiadaczowi PRAWO (nie obowiązek) kupna instrumentu bazowego od wystawcy." },
      { q: "Wystawca opcji sprzedaży:", o:["w terminie wygaśnięcia musi kupić od nabywcy instrument bazowy, płacąc za każdą sztukę kwotę równą kursowi rozliczeniowemu opcji","jeżeli są to opcje europejskie, jest zobligowany do wykonania opcji w dowolnym terminie","jeżeli opcja jest wykonywana, płaci za każdą sztukę instrumentu bazowego kwotę równą kursowi wykonania opcji"], a:2, e:"Przy wykonaniu put wystawca kupuje instrument bazowy po kursie wykonania (a nie rozliczeniowym)." },
      { q: "Które z poniższych opcji są notowane na GPW?", o:["opcje na indeks WIG","opcje na obligacje","opcje na indeks WIG20"], a:2, e:"Na GPW notowane są opcje na indeks WIG20." },
      { q: "Ile maksymalnie może stracić nabywca opcji kupna?", o:["straty, tak jak w przypadku kontraktów terminowych, są nieograniczone","straty są ograniczone do wysokości zapłaconej za opcję premii","strata jest ograniczona do wysokości środków znajdujących się w depozycie"], a:1, e:"Nabywca opcji ryzykuje wyłącznie zapłaconą premię — nie wnosi depozytu." },
      { q: "Wystawca opcji kupna zarabia, jeżeli:", o:["wartość instrumentu bazowego rośnie","zmienność instrumentu bazowego spada","zmienność instrumentu bazowego rośnie"], a:1, e:"Spadek zmienności obniża wartość opcji — korzyść dla wystawcy (otrzymał premię)." },
      { q: "Nabywca opcji sprzedaży zarabia, jeżeli:", o:["zmienność instrumentu bazowego rośnie","wartość instrumentu bazowego nie zmienia się","wartość instrumentu bazowego rośnie"], a:0, e:"Wzrost zmienności podnosi wartość opcji put — korzyść dla nabywcy." },
      { q: "Ile wynosi mnożnik dla opcji na WIG20?", o:["1 zł","10 zł","5 zł"], a:1, e:"Mnożnik opcji na WIG20 wynosi 10 zł za punkt indeksowy." },
      { q: "Podaj, w jakim miesiącu wygasa opcja OW2OC1723OO oraz jakiego jest typu:", o:["opcja kupna wygasająca w grudniu","opcja sprzedaży wygasająca w marcu","opcja kupna wygasająca w marcu"], a:2, e:"W oznaczeniu litera „C” to opcja kupna (call), a kod miesiąca wskazuje marzec." },
      { q: "Kupiliśmy opcję kupna na akcje XYZ z kursem wykonania 130 zł. Kurs transakcji (premia) 2,50 zł, 100 akcji na opcję. W terminie wygaśnięcia kurs akcji 135 zł. Jaką stopę zwrotu uzyskaliśmy?", o:["–100%","+100%","+200%"], a:1, e:"Wartość w terminie: (135−130)=5 zł; zapłacono 2,50 zł. Zysk 2,50 zł na 2,50 zł → +100%." },
      { q: "Kupiliśmy opcję sprzedaży na WIG20 z kursem wykonania 3.300 pkt. Kurs transakcji 70 pkt. Kurs rozliczeniowy 3.380 pkt. Jaka jest kwota zysku/straty?", o:["–700 zł","+800 zł","+100 zł"], a:0, e:"Put OTM (3380 > 3300) → wartość 0. Strata = 70 pkt × 10 zł = 700 zł → –700 zł." },
      { q: "Ile wynosi wartość wewnętrzna opcji kupna na WIG20 o kursie wykonania 3.200 pkt, jeżeli wartość indeksu WIG20 wynosi 3.280 pkt, a kurs opcji 90 pkt?", o:["0 pkt","80 pkt","10 pkt"], a:1, e:"Wartość wewnętrzna call = max(3280−3200; 0) = 80 pkt." },
      { q: "Ile wynosi wartość czasowa opcji sprzedaży na WIG20 o kursie wykonania 3.600 pkt, jeżeli indeks wynosi 3.670 pkt, a kurs opcji 50 pkt?", o:["70 pkt","20 pkt","50 pkt"], a:2, e:"Wartość wewnętrzna put = max(3600−3670;0)=0, więc wartość czasowa = cała premia = 50 pkt." },
      { q: "Kupiłeś opcję kupna na WIG20 z kursem wykonania 3.300 pkt. Kurs transakcji 120 pkt, indeks 3.350. Indeks rośnie do 3.420 pkt, premia rośnie do 200 pkt — sprzedajesz opcję. Kwota zysku/straty?", o:["800 zł","700 zł","1.200 zł"], a:0, e:"Zysk na premii: (200−120) = 80 pkt × 10 zł = 800 zł." },
      { q: "Spread byka: opcja kupna na WIG20 (kurs wykonania 3.000) kupiona za 100 pkt + opcja kupna (kurs wykonania 3.200) wystawiona za 40 pkt. Kurs rozliczeniowy 3.180 pkt. Zysk/strata?", o:["1.800 zł","1.200 zł","–600 zł"], a:1, e:"Long call: 180−100=80; short call: 0+40=40 → razem 120 pkt × 10 zł = 1.200 zł." },
      { q: "Długi stelaż: opcja kupna (kurs wykonania 3.600) za 80 pkt + opcja sprzedaży (kurs wykonania 3.600) za 70 pkt. Podaj punkty opłacalności:", o:["3.520 pkt oraz 3.670 pkt","3.450 pkt oraz 3.750 pkt","3.530 pkt oraz 3.680 pkt"], a:1, e:"Koszt = 80+70 = 150 pkt. Progi: 3.600 ± 150 = 3.450 i 3.750 pkt." },
      { q: "Arbitraż opcje/kontrakty na WIG20 — budujesz syntetyczną długą pozycję z opcji (kurs wykonania 3.600). Arkusz: opcja kupna kupno 240 / sprzedaż 250; opcja sprzedaży kupno 120 / sprzedaż 129; kontrakt kupno 3.748 / sprzedaż 3.749. Ile można zarobić?", o:["180 zł","190 zł","370 zł"], a:0, e:"Syntetyk: kupno call 250 − sprzedaż put 120 daje kurs ~3.600+130=3.730; sprzedaż kontraktu po 3.748 → różnica 18 pkt × 10 zł = 180 zł." }
    ]
  },

  /* ===== TEST POWTÓRKOWY (19 pytań przekrojowych) ===== */
  powtorka19: {
    title: "📝 Test powtórkowy — 19 pytań",
    desc: "Przekrojowa powtórka: pochodne, futures, opcje, greckie, wycena.",
    icon: "📝",
    questions: [
      { q: "Instrumenty pochodne wykorzystujemy w celu:", o:["Osiągania korzyści z różnicy cen instrumentu bazowego","Spekulacji","Zabezpieczenia portfeli inwestycyjnych przed ryzykiem zmiany ceny instrumentu bazowego","Wszystkie odpowiedzi A, B, C są prawidłowe"], a:3, e:"Pochodne służą do spekulacji (B), zabezpieczania/hedgingu portfela (C) oraz wykorzystywania zmian cen instrumentu bazowego (A) — wszystkie są prawdziwe, więc poprawne jest D." },
      { q: "Zarządzanie ryzykiem obejmuje:", o:["Rozpoznawanie rodzajów ryzyka, na jakie narażony jest dany podmiot","Pomiar ryzyka","A i B są poprawne","A i B są fałszywe"], a:2, e:"Zarządzanie ryzykiem obejmuje i identyfikację rodzajów ryzyka (A), i jego pomiar (B), więc poprawne jest C. D jest błędne, bo oba zdania są prawdziwe." },
      { q: "Wystawca kontraktu forward osiąga zysk, gdy rynkowa cena instrumentu bazowego w chwili realizacji kontraktu:", o:["Jest niższa od ceny forward","Jest wyższa od ceny forward","Nie różni się od ceny wykonania kontraktu","Różni się od ceny wykonania kontraktu"], a:0, e:"Wystawca zajmuje pozycję krótką — zarabia, gdy cena rynkowa jest NIŻSZA od ceny forward (sprzedaje drożej niż rynek). B opisuje zysk nabywcy (pozycji długiej)." },
      { q: "Instrumenty pochodne występują:", o:["Na rynku instrumentów pochodnych","Na rynku giełdowym","Na rynku pozagiełdowym","Wszystkie odpowiedzi A, B, C są poprawne"], a:3, e:"Pochodne handlowane są zarówno na rynku giełdowym (ETD), jak i pozagiełdowym (OTC), więc poprawne jest D." },
      { q: "Wycena (wartość teoretyczna) kontraktów futures:", o:["Nie zależy od aktualnej ceny instrumentu bazowego","Nie zależy od wysokości stopy procentowej pozbawionej ryzyka","Zależy od liczby dni do terminu wygaśnięcia kontraktu","Wszystkie odpowiedzi A, B, C są poprawne"], a:2, e:"Cena futures zależy od ceny spot, stopy wolnej od ryzyka ORAZ liczby dni do wygaśnięcia. A i B są fałszywe (zależy i od ceny spot, i od stopy), więc prawdziwe jest tylko C." },
      { q: "Cechy charakterystyczne giełdowych kontraktów futures to:", o:["Konieczność wniesienia depozytu zabezpieczającego jedynie przez wystawcę kontraktu","Konieczność wniesienia depozytu zabezpieczającego jedynie przez nabywcę kontraktu","A i B są prawidłowe","A i B są fałszywe"], a:3, e:"Depozyt zabezpieczający wnoszą OBIE strony kontraktu futures, więc zarówno A, jak i B są fałszywe → D." },
      { q: "Ryzyko kontraktu futures:", o:["Wynika z różnicy między ceną futures a ceną spot instrumentu bazowego","Jest to tzw. ryzyko bazy","A i B są prawidłowe","A i B są fałszywe"], a:2, e:"Baza = cena futures − cena spot; ryzyko bazy wynika właśnie z tej różnicy. Oba zdania A i B są prawdziwe → C." },
      { q: "Portfel akcji WIG20 wart 105 000 zł, kurs kontraktu futures 2400 pkt (mnożnik 20 zł). Inwestor przewiduje spadek i chce się zabezpieczyć:", o:["Powinien sprzedać 3 wrześniowe kontrakty na indeks WIG20","Powinien kupić 3 wrześniowe kontrakty na indeks WIG20","Powinien wystawić 2 wrześniowe kontrakty na indeks WIG20","Żadna z odpowiedzi A, B, C nie jest poprawna"], a:2, e:"Wartość 1 kontraktu = 2400×20 = 48 000 zł; 105 000/48 000 ≈ 2,19 → 2 kontrakty. Długą pozycję w akcjach zabezpiecza pozycja KRÓTKA (wystawienie) → C. A ma złą liczbę (3), B — zły kierunek (kupno)." },
      { q: "Bieżący kurs futures WIG20 wynosi 2350 pkt, a wycena teoretyczna 2345 pkt. Jaka strategia arbitrażowa?", o:["Długa na rynku terminowym i krótka na kasowym","Krótka na rynku terminowym i długa na kasowym","Obie fałszywe","Obie prawdziwe"], a:1, e:"Futures (2350) > wartość teoretyczna (2345) → jest przewartościowany. Arbitraż: sprzedaj drogi futures (krótka na terminowym) i kup tani instrument bazowy (długa na kasowym) → B." },
      { q: "Zabezpieczenie pozycji importera polega na:", o:["Zajęciu krótkiej pozycji w walutowych kontraktach futures","Zajęciu długiej pozycji w walutowych kontraktach futures","A i B są prawidłowe","A i B są fałszywe"], a:1, e:"Importer obawia się WZROSTU kursu waluty, którą zapłaci w przyszłości — zabezpiecza się pozycją DŁUGĄ (kupno) → B. Krótka pozycja (A) zabezpiecza eksportera." },
      { q: "Czynniki wpływające na wartość wewnętrzną opcji to:", o:["Czas do wygaśnięcia","Cena wykonania opcji","Zmienność instrumentu bazowego","A, B i C są prawidłowe"], a:1, e:"Wartość wewnętrzna zależy wyłącznie od relacji ceny instrumentu bazowego i ceny wykonania (B). Czas (A) i zmienność (C) wpływają na wartość CZASOWĄ, nie wewnętrzną — dlatego D jest błędne." },
      { q: "Opcja call: cena wykonania (strike) 104, cena akcji 101. Co się dzieje w terminie wygaśnięcia?", o:["Zysk z wykonania","Wystawca traci premię","Opcja bezwartościowa","Zysk ze sprzedaży opcji"], a:2, e:"101 < 104 → call jest out-of-the-money i wygasa bezwartościowo (C). Wykonanie nie daje zysku (A błędne); wystawca premię ZACHOWUJE, a nie traci (B błędne)." },
      { q: "Opcja put: cena wykonania (strike) 2500, indeks 2350. Kto zyskuje?", o:["Wystawca zyskuje","Nabywca zyskuje","A i B","Żadne"], a:1, e:"2350 < 2500 → put jest in-the-money, więc nabywca realizuje zysk (B). Wystawca w tej sytuacji traci, dlatego A i C są błędne." },
      { q: "Kontrakt swap:", o:["Ma ryzyko niesymetryczne","Jest instrumentem giełdowym","To zawsze wymiana wyłącznie stałych płatności","A, B, C są fałszywe"], a:3, e:"Swap ma ryzyko SYMETRYCZNE, jest instrumentem POZAGIEŁDOWYM (OTC) i nie zawsze dotyczy tylko stałych płatności (np. IRS: stała za zmienną). Zatem A, B i C są fałszywe → D." },
      { q: "Współczynnik delta to:", o:["Reakcja ceny opcji na zmienność","Miara wrażliwości ceny opcji na zmianę ceny instrumentu bazowego","A i B","Fałszywe"], a:1, e:"Delta mierzy zmianę ceny opcji względem zmiany ceny instrumentu bazowego (B). Reakcja na zmienność (A) to vega, nie delta — dlatego C ('A i B') jest błędne." },
      { q: "Współczynnik theta to:", o:["Wrażliwość na stopy procentowe","Wrażliwość na cenę instrumentu bazowego","A i B","Fałszywe"], a:3, e:"Theta mierzy wpływ upływu czasu na cenę opcji. Wrażliwość na stopy (A) to rho, a wrażliwość na cenę bazowego (B) to delta — oba błędne → D." },
      { q: "Model dwumianowy (CRR) pozwala wyceniać:", o:["Tylko opcje europejskie call","Opcje europejskie na dowolny instrument","Dowolne instrumenty pochodne (w tym opcje amerykańskie)","A, B, C"], a:2, e:"Model dwumianowy jest elastyczny i wycenia różne instrumenty pochodne, także opcje amerykańskie (C). Odpowiedzi A i B nadmiernie go zawężają." },
      { q: "Prawdopodobieństwo arbitrażowe (risk-neutral) wykorzystywane jest:", o:["W wycenie opcji, m.in. w modelu Blacka–Scholesa","Tylko w modelu dwumianowym","Niezależnie od stóp procentowych","Fałszywe"], a:0, e:"Prawdopodobieństwo risk-neutral leży u podstaw wyceny pochodnych, m.in. w Blacku–Scholesie (A). 'Tylko model dwumianowy' (B) jest zbyt wąskie; zależy ono od stopy wolnej od ryzyka, więc C jest błędne." },
      { q: "Wycena opcji call w modelu Blacka–Scholesa:", o:["Zależy od ceny wykonania (strike)","Zależy od bieżącej ceny opcji","Nie zależy od czasu do wygaśnięcia","A i B"], a:0, e:"Cena call w B–S zależy m.in. od ceny wykonania (A). Model WYZNACZA cenę opcji, więc nie 'zależy od bieżącej ceny opcji' (B błędne); zależy też od czasu, dlatego C jest fałszywe." }
    ]
  }
};
